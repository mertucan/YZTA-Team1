...
def _recompute_menu_totals(menu_id: int) -> None:
    db = get_db()
    items = db.table("weekly_menu_items").select("estimated_cost, calories, protein, iron").eq("weekly_menu_id", menu_id).execute()
    total_cost = sum(float(i.get("estimated_cost") or 0) for i in items.data)
...
def add_meal_item(menu_id: int, payload: WeeklyMenuMealItemCreate):
    ...
    estimated_cost = round(total_ingredient_cost / portions, 2)
