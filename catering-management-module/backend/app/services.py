...
from app.models import License, Role, RoleModel, University, UserProfile
from app.schemas import UniversityCreate, UserCreate


def ensure_active_license(db: Session, company_id: uuid.UUID) -> License:
    today = date.today()
...
def create_university_for_company(db: Session, company_id: uuid.UUID, payload: UniversityCreate) -> University:
...
def create_user_for_company(db: Session, company_id: uuid.UUID, payload: UserCreate) -> UserProfile:
    license_row = ensure_active_license(db, company_id)
