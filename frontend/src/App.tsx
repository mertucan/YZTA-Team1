...
import type { StatisticsResponse, WeeklyMenuInfo, MenuComparisonResponse, SystemNotification } from "./types";
import "./styles.css";
...
const getUnisByRegion = (region: string): string[] => {
  switch (region) {
    case "Marmara":
...
const getUniStudentCount = (uniName: string): number => {
...
function App() {
  ...
  const [activeTab, setActiveTab] = useState<"signals" | "general" | "research" | "history" | "reports" | "notifications" | "settings">("signals");

  // Notifications State
  const [notifications, setNotifications] = useState<SystemNotification[]>([]);

  // Settings States
  const [anemiaThreshold, setAnemiaThreshold] = useState<number>(12.0);
  const [obesityThreshold, setObesityThreshold] = useState<number>(25.0);
  const [dataRefreshInterval, setDataRefreshInterval] = useState<string>("Günlük");
  const [settingsSaved, setSettingsSaved] = useState<boolean>(false);

  // Tab 2 Selected Disease state (dynamic)
  const [selectedDisease, setSelectedDisease] = useState<string>("anemi");
  ...
  // -------------------------------------------------------------
  const [activeSubTab, setActiveSubTab] = useState<"period" | "menu" | "trend">("period");

  // Global filters for period and trend sub-tabs
  const [filterRegion, setFilterRegion] = useState<string>("Tümü");
  const [filterUniversity, setFilterUniversity] = useState<string>("Tümü");
  const [filterGender, setFilterGender] = useState<string>("Tümü");
  const [filterAge, setFilterAge] = useState<string>("Tümü");

  // Sub-tab 1: Period comparison states
  const [compareType, setCompareType] = useState<string>("Anemi Eğilim Analizi");
  const [compPeriod1Start, setCompPeriod1Start] = useState<string>("2026-01-01");
  const [compPeriod1End, setCompPeriod1End] = useState<string>("2026-03-31");
  const [compPeriod2Start, setCompPeriod2Start] = useState<string>("2026-04-01");
  const [compPeriod2End, setCompPeriod2End] = useState<string>("2026-06-30");
  const [runningComparison, setRunningComparison] = useState<boolean>(false);
  const [comparisonResults, setComparisonResults] = useState<any[] | null>(null);

  // Sub-tab 2: Menu comparison states
  const [menusList, setMenusList] = useState<WeeklyMenuInfo[]>([]);
  const getMenuDisplayName = (m: WeeklyMenuInfo) => {
  ...
  const [menu1Id, setMenu1Id] = useState<number>(0);
  const [menu2Id, setMenu2Id] = useState<number>(0);
  const [menuComparisonResult, setMenuComparisonResult] = useState<MenuComparisonResponse | null>(null);
  const [runningMenuComparison, setRunningMenuComparison] = useState<boolean>(false);
  const [menuComparisonError, setMenuComparisonError] = useState<string | null>(null);

  // Sub-tab 3: Trend query states
  const [trendStart, setTrendStart] = useState<string>("2026-01-01");
  const [trendEnd, setTrendEnd] = useState<string>("2026-06-30");
  const [trendQueryResults, setTrendQueryResults] = useState<any[] | null>(null);
  const [runningTrendQuery, setRunningTrendQuery] = useState<boolean>(false);
  const [trendQueryError, setTrendQueryError] = useState<string | null>(null);

  // ─── RAPOR ÖNİZLEME STATELERİ ───
  const [reportTypeForPreview, setReportTypeForPreview] = useState<"custom" | "report1" | "report2" | "report3" | null>(null);
  ...
  const loadInitialData = () => {
    ...
    getMenus()
      ...
      .catch((err: Error) => console.error("Menüler yüklenemedi", err));

    getNotifications()
      .then(setNotifications)
      .catch((err: Error) => console.error("Bildirimler yüklenemedi", err));
  };
  ...
  const handleDeleteNotification = (id: number) => {
    deleteNotification(id)
      ...
      .catch((err: Error) => console.error("Bildirim silinemedi", err));
  };
  ...
  const handleClearAllNotifications = () => {
    clearAllNotifications()
      ...
      .catch((err: Error) => console.error("Bildirimler temizlenemedi", err));
  };
  ...
  const trendData = useMemo(() => {
    if (data && data.trend && data.trend.length > 0) {
      return data.trend.map(pt => {
        ...
        const ptDynamic: Record<string, any> = {
          name: name,
          period: pt.period,
  ...
  const handleMenuCompare = async (e: React.FormEvent) => {
    e.preventDefault();
  ...
  const handleTrendQuery = async (e: React.FormEvent) => {
    e.preventDefault();
  ...
  // ─── PDF RAPOR ÜRETICI ───
  const printPdfInIframe = async (htmlContent: string) => {
  ...
  const generateReportHtml = (
    type: "custom" | "report1" | "report2" | "report3" | null,
  ...
  return (
    <div className="app-shell">
      ...
      <aside className="sidebar">
        <div className="sidebar-logo" style={{ cursor: "pointer" }} onClick={() => setActiveTab("signals")}>
          <Apple size={20} />
        </div>
        <nav className="sidebar-nav">
          <div
            className={`sidebar-icon ${activeTab === "signals" ? "active" : ""}`}
            title="Erken Eğilim Sinyalleri"
            onClick={() => setActiveTab("signals")}
          >
            <Home size={18} />
          </div>
          <div
            className={`sidebar-icon ${activeTab === "general" ? "active" : ""}`}
            title="Genel Beslenme Eğilimleri"
            onClick={() => setActiveTab("general")}
          >
            <BarChart2 size={18} />
          </div>
          <div
            className={`sidebar-icon ${activeTab === "research" ? "active" : ""}`}
            title="Araştırmacı Analiz Paneli"
          ...
          <div
            className={`sidebar-icon ${activeTab === "reports" ? "active" : ""}`}
            title="Raporlar"
                  ...
                  {Object.entries(
                    ...
                    return (
                      <div className="flag-item-row" key={key}>
                        ...
                        <span className="flag-count">{val as number} vaka</span>
                      </div>
                    );
                  })}
                </div>
        ...
        <section style={{ animation: "fadeIn 0.3s ease" }}>
          ...
          <div className="summary-grid">
            ...
            <div className="summary-card">
              ...
              <span className="card-value">
                {data?.active_health_flags
                  ? formatNumber(Object.values(data.active_health_flags).reduce((a: number, b) => a + (b as number), 0))
                  : "—"}
              </span>
