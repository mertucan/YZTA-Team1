...
export default function Absences() {
  ...
  const refresh = () => getAbsences().then(setItems).finally(() => setLoading(false));
  useEffect(() => { refresh(); }, []);

  const handleAdd = async () => {
    if (!form.student_id || !form.absence_date) return;
  ...
  return (
    <div>
      <div style={{ marginBottom: 20 }}>
        <div style={{ fontSize: 20, fontWeight: 600 }}>✈️ Devamsızlık Kayıtları</div>
        <div style={{ fontSize: 13, color: "var(--text2)", marginTop: 3 }}>Öğrencilerin yemekhanede olmayacağı günler</div>
      </div>
      ...
      <div style={card}>
        ...
        <div style={{ padding: 18, display: "grid", gridTemplateColumns: "1fr 1fr auto", gap: 10, alignItems: "end" }}>
          <div>
            ...
            <input type="number" value={form.student_id || ""} placeholder="Öğrenci ID" onChange={(e) => setForm({ ...form, student_id: parseInt(e.target.value) || 0 })} style={input} />
          </div>
          <div>
            <div style={fieldLabel}>Tarih</div>
            <input type="date" value={form.absence_date} onChange={(e) => setForm({ ...form, absence_date: e.target.value })} style={input} />
          </div>
      ...
      <div style={card}>
        ...
        {loading ? <div style={{ padding: 24, color: "var(--text3)" }}>Yükleniyor...</div> : (
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            <thead>
              <tr>{["Öğrenci ID", "Tarih", ""].map((h) => <th key={h} style={th}>{h}</th>)}</tr>
            </thead>
            <tbody>
              {items.map((a) => (
                <tr key={a.id}>
                  <td style={td}>{a.student_id}</td>
                  <td style={{ ...td, fontFamily: "var(--mono)" }}>{a.absence_date}</td>
                  <td style={td}><button onClick={() => deleteAbsence(a.id).then(refresh)} style={btnSm}>Sil</button></td>
                </tr>
              ))}
            </tbody>
          </table>
